...
- [ ] A.2 确认云函数 OpenAPI 写接口（setfuncconfig 或等价）或书面接受 `config.json`+deploy 降级
  - _需求: 2, 4
...
- [ ] B.1 新增 `query_msg_push` / `manage_msg_push`（qbase 封装 + 幂等 merge + 写确认）
  - _需求: 1
  - **开发时使用 worktree 隔离：** 是（main 仓独立 worktree）

- [ ] B.2 新增 `query_cloud_call` / `manage_cloud_call`（按 A.2 契约）
  - _需求: 2
  - **开发时使用 worktree 隔离：** 是
...
- [ ] E.1 幂等与 7 事件默认订阅手工/e2e
  - _需求: 1
  - **开发时使用 worktree 隔离：** 是（验证环境独立）

- [ ] E.2 云调用 bind/unbind 与查询一致
  - _需求: 2
