...
* **ci**: `Internal Docs Guard` now reads file contents as well, and the two internal references that had been sitting in the public tree since before the metadata tier existed are gone. That tier covered PR titles, bodies and commit messages but never opened a committed file, so an internal hostname or address could sit in the repository indefinitely — and two did: a severity checklist that named the internal test endpoints used to prove SSRF reachability, and a doc-sync note that named the internal Git remote together with the local clone path. `scripts/check-internal-refs.mjs` now walks every tracked file line by line and reports `path:line`, rejecting internal hostnames and internal network addresses. The rules stay deliberately narrow — private ranges, `localhost` and `169.254.169.254` are ordinary content in the documentation and in the SSRF guard's own tests, so matching them would be a constant source of false positives. It scans the whole tracked tree rather than the diff, so residue on `main` surfaces on the next pull request instead of waiting for someone to edit that particular file; the tree is clean at this commit, which is what makes turning the check on safe. Both files keep their meaning without the addresses: the checklist asks for an internal-only hostname and IP chosen from the reader's own network, and the doc-sync note names the repository and says the clone path is machine-specific.
