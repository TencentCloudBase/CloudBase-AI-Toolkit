...
/**
 * Pass-through dirs may contain local generated payloads that are gitignored
 * on main (for example config/codebuddy-plugin/skills/, regenerated by
 * sync-codebuddy-plugin.ts and force-added only on chore/codebuddy_plugin).
 * Copying the working tree wholesale makes local and CI compat outputs diverge.
 */
...
function listGitTrackedFiles(relativeDir) {
...
function copyPassThroughDirs(outputDir) {
  for (const dirName of PASS_THROUGH_DIRS) {
    ...
    const relativeDir = path.posix.join("config", dirName);
    const trackedFiles = listGitTrackedFiles(relativeDir);
    ...
    for (const trackedFile of trackedFiles) {
      ...
      const relativeInside = trackedFile.slice(prefix.length);
      copyFile(path.join(ROOT_DIR, trackedFile), path.join(outputDir, dirName, relativeInside));
    }
  }
}
