...
| 工具标识                  | 功能描述                                  | 核心参数                                                                                     |
|---------------------------|-----------------------------------------|---------------------------------------------------------------------------------------------|
| `uploadFiles`             | 上传文件到静态网站托管                    | `localPath`（选填，本地文件或文件夹绝对路径），`cloudPath`（选填，静态托管目标路径；部署到根目录 `/` 时请传空字符串或省略，且不要以 `/` 开头），`files`（选填，多文件上传配置），`ignore`（选填，忽略文件模式）；成功后会返回 `accessUrl`、`message`、`deploymentStatus`，可配合 `findFiles` 继续核验 |
