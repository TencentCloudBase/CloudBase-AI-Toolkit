...
| 工具标识                  | 功能描述                                  | 核心参数                                                                                     |
|---------------------------|-----------------------------------------|---------------------------------------------------------------------------------------------|
| `uploadFiles`             | 上传构建产物到静态网站托管（仅 Web 部署，不用于云存储） | `localPath`（选填，上传完整构建目录如 `dist/`，不要只传 `index.html`），`cloudPath`（选填，相对托管根目录、不要前导 `/`；子目录部署时需与 `base/publicPath/assetPrefix` 对应的绝对路径配置一致；云存储对象请改用 `manageStorage`），`files`（选填，多文件上传配置），`ignore`（选填，忽略文件模式） |
