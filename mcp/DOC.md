...
| 工具标识                  | 功能描述                                  | 核心参数                                                                                     |
|---------------------------|-----------------------------------------|---------------------------------------------------------------------------------------------|
| `uploadFiles`             | 上传文件到静态网站托管                    | `localPath`（选填，本地文件或文件夹路径），`cloudPath`（选填，静态托管云端路径，相对于托管根目录；根目录留空，子目录传 `vite-test` 这类不带前导 `/` 的路径；若访问路径是 `/vite-test`，需同时确保构建工具 `base` / `publicPath` / `assetPrefix` 与部署路径一致，并上传完整 `dist/` 目录），`files`（选填，多文件上传配置，`files[].cloudPath` 同样必须是不带前导 `/` 的相对路径），`ignore`（选填，忽略文件模式） |
| `listFiles`               | 获取静态网站托管的文件列表                | 无                                                                                           |
| `deleteFiles`             | 删除静态网站托管的文件或文件夹            | `cloudPath`（必填，托管云端路径，相对于托管根目录，不要以 `/` 开头；根目录传空字符串），`isDir`（选填，是否为文件夹，默认为 `false`）     |
| `findFiles`               | 搜索静态网站托管的文件                    | `prefix`（必填，托管路径前缀，相对于托管根目录，不要以 `/` 开头；空字符串表示根目录前缀），`marker`（选填，起始对象键标记），`maxKeys`（选填，单次返回最大条目数） |
