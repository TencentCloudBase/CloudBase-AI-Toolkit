...
| 工具标识                  | 功能描述                                  | 核心参数                                                                                     |
|---------------------------|-----------------------------------------|---------------------------------------------------------------------------------------------|
| `uploadFiles`             | 上传文件到静态网站托管                    | `localPath`（选填，本地文件或文件夹路径），`cloudPath`（选填，静态托管云端路径，使用托管路径语义而不是完整 URL；`/` 或留空表示托管根目录，`vite-test`、`/vite-test`、`/vite-test/` 表示同一个子目录），`files`（选填，多文件上传配置，`files[].cloudPath` 同样使用托管路径语义，服务端会自动规范化），`ignore`（选填，忽略文件模式） |
| `listFiles`               | 获取静态网站托管的文件列表                | 无                                                                                           |
| `deleteFiles`             | 删除静态网站托管的文件或文件夹            | `cloudPath`（必填，托管云端路径，使用托管路径语义；`/` 表示根目录，`vite-test`、`/vite-test`、`/vite-test/` 表示同一个目录），`isDir`（选填，是否为文件夹，默认为 `false`）     |
| `findFiles`               | 搜索静态网站托管的文件                    | `prefix`（必填，托管路径前缀，使用托管路径语义；`/` 表示根目录前缀，`vite-test` 和 `/vite-test` 会查询同一前缀），`marker`（选填，起始对象键标记），`maxKeys`（选填，单次返回最大条目数） |
