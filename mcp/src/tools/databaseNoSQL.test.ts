...
describe("NoSQL database tools", () => {
  ...
  // Regression: issue #994 — 后端 Code 说配额、Message 却说"写太多"，
  // 原样透传会把用户和 AI 一起误导到"减少单次写入条数"上。
  ...
  it("writeNoSqlDatabaseContent should translate write quota overrun into actionable guidance", async () => {
    const { tools } = createMockServer();
    mockCommonServiceCall.mockRejectedValueOnce(new Error(WRITE_OVERRUN_MESSAGE));

    await expect(
      tools.writeNoSqlDatabaseContent.handler({
        action: "insert",
        collectionName: "t_nosql_products",
        documents: [{ name: "note-1" }],
      }),
    ).rejects.toThrow(/数据库写入配额已用尽[\s\S]*与单次写入的条数或文档大小无关[\s\S]*30,000/);
  });

  it("writeNoSqlDatabaseContent should keep backend error code so error-code troubleshooting can match docs", async () => {
    const { tools } = createMockServer();
    mockCommonServiceCall.mockRejectedValueOnce(new Error(WRITE_OVERRUN_MESSAGE));

    const error: any = await tools.writeNoSqlDatabaseContent
      .handler({
        action: "insert",
        collectionName: "t_nosql_products",
        documents: [{ name: "note-1" }],
    ...
    expect(error.code).toBe("LimitExceeded.OutOfWriteRequestQuota");
    expect(error.message).toContain("LimitExceeded.OutOfWriteRequestQuota");
    expect(error.message).toContain(WRITE_OVERRUN_MESSAGE);
  });

  it("writeNoSqlDatabaseContent should normalize SDK requestID onto requestId for failed writes", async () => {
    const { tools } = createMockServer();
    const apiError: any = new Error(WRITE_OVERRUN_MESSAGE);
    apiError.requestID = "req-backend-994";
    mockCommonServiceCall.mockRejectedValueOnce(apiError);
    ...
    expect(error.requestId).toBe("req-backend-994");
  });

  it("writeNoSqlDatabaseContent should leave unrelated write errors untouched", async () => {
    const { tools } = createMockServer();
    mockCommonServiceCall.mockRejectedValueOnce(
      new Error("[PutItem] Collection does not exist"),
    );
    ...
    ).rejects.toThrow("[PutItem] Collection does not exist");
  });
});
