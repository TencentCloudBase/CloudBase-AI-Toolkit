...
export function registerEnvTools(server: ExtendedMcpServer) {
  ...
  server.registerTool?.(
    "envQuery",
    {
      title: "环境查询",
      description:
        "查询云开发环境相关信息，支持查询环境列表、当前环境信息、安全域名和静态网站托管配置。（原工具名：listEnvs/getEnvInfo/getEnvAuthDomains/getWebsiteConfig，为兼容旧AI规则可继续使用这些名称）当 action=list 时，标准返回字段为 EnvId、Alias、Status、EnvType、Region、PackageId、PackageName、IsDefault，并支持通过 fields 白名单裁剪这些字段；aliasExact=true 时会按别名精确筛选，避免把前缀相近的环境误当作候选；即使传入 envId，action=list 也只返回摘要，不会返回完整资源明细或 expiry。如需查询指定环境的详细信息（包括资源字段和计费信息），请使用 action=info 并传入目标环境的 envId 参数。",
      inputSchema: {
        action: z
          .enum(["list", "info", "domains", "hosting"])
          .describe(
            "查询类型：list=环境列表/摘要筛选（支持通过 envId 筛选，返回 EnvId、Alias、Status、EnvType、Region、PackageId、PackageName、IsDefault），info=指定环境的详细信息（需传入 envId，返回资源字段和计费信息），domains=安全域名列表，hosting=静态网站托管配置",
          ),
        ...
        envId: z.string().optional().describe("按环境 ID 筛选。action=list 时可选（返回摘要）；action=info 时必选（返回该环境的详细信息，包含资源字段和计费信息）。如需查询指定环境的详细信息，请使用 action=info 并传入对应 envId"),
