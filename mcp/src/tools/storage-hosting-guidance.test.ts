...
describe("storage and hosting tool guidance", () => {
  it("should clearly separate static hosting uploads from cloud storage uploads", () => {
    ...
    expect(tools.uploadFiles.meta.inputSchema.cloudPath.description).toContain("相对于托管根目录");
    expect(tools.uploadFiles.meta.inputSchema.cloudPath.description).toContain("不要传 `/vite-test`");
    expect(tools.uploadFiles.meta.inputSchema.cloudPath.description).toContain("上传完整构建目录（通常是 `dist/`）");
    expect(tools.uploadFiles.meta.inputSchema.cloudPath.description).toContain("云存储对象路径请改用 manageStorage");
    expect(tools.uploadFiles.meta.inputSchema.files.description).toContain("不要以 `/` 开头");
    expect(tools.deleteFiles.meta.inputSchema.cloudPath.description).toContain("不要以 `/` 开头");
    expect(tools.findFiles.meta.inputSchema.prefix.description).toContain("空字符串表示根目录前缀");
