...
describe("storage and hosting tool guidance", () => {
  it("should clearly separate static hosting uploads from cloud storage uploads", () => {
    ...
    expect(tools.uploadFiles.meta.description).toContain("调用前必须显式确认");
    expect(tools.uploadFiles.meta.description).toContain("不要用 './' 或空字符串");
    expect(tools.uploadFiles.meta.inputSchema.localPath.description).toContain("不能只上传 index.html");
    expect(tools.uploadFiles.meta.inputSchema.cloudPath.description).toContain("不要前导 '/'");
    expect(tools.uploadFiles.meta.inputSchema.cloudPath.description).toContain("base/publicPath/assetPrefix");
