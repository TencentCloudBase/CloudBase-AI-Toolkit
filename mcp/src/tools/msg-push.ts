...
/** qbase 管理 CGI 地址（与微信云开发控制台 callbackconfig.ts 使用的一致） */
const QBASE_GET_APP_CONFIG = "https://servicewechat.com/wxa-dev-qbase/getappconfig";
const QBASE_UPLOAD_APP_CONFIG =
  "https://servicewechat.com/wxa-dev-qbase/uploadappconfig";
const QBASE_GET_CALLBACK_SUPPORT_LIST =
  "https://servicewechat.com/wxa-dev-qbase/route/getcallbacksupportlist";
const QBASE_GET_CONTAINER_CONFIG =
  "https://servicewechat.com/wxa-dev-qbase/getcontainercallbackconfig";
const QBASE_SET_CONTAINER_CONFIG =
  "https://servicewechat.com/wxa-dev-qbase/setcontainercallbackconfig";
...
function getTransport(server: ExtendedMcpServer): MsgPushRequestFn | undefined {
  return server.pluginOptions?.msgPush?.requestFn;
}
...
async function callQbase(
  ...
  if (!requestFn) {
    throw new QbaseError(
      ...
      "未注入 qbase 请求通道（pluginOptions.msgPush.requestFn）",
    );
  }
  const options: MsgPushRequestFnOptions = { url, method, body, appid };
  try {
    return await requestFn(options);
  } catch (e) {
    throw new QbaseError(
      "TRANSPORT_ERROR",
      `qbase 请求失败(${url}): ${e instanceof Error ? e.message : String(e)}`,
    );
  }
}
...
async function readCallbackConfig(
  ...
  const resp = await callQbase(server, appid, QBASE_GET_APP_CONFIG, { type: 1 });
...
async function fetchSupportedEvents(
  ...
  const resp = await callQbase(server, appid, QBASE_GET_CALLBACK_SUPPORT_LIST, {});
  const ret = resp.base_resp?.ret;
  if (ret !== undefined && ret !== 0) {
    throw new QbaseError(
      ret,
...
async function uploadCallbackConfig(
  ...
  const resp = await callQbase(server, appid, QBASE_UPLOAD_APP_CONFIG, {
    type: 1,
...
async function readContainerConfig(
  ...
  const resp = await callQbase(server, appid, QBASE_GET_CONTAINER_CONFIG, {}, "get");
  const ret = resp.base_resp?.ret;
  if (ret !== undefined && ret !== 0) {
    if (ret === RET_CONFIG_NOT_EXISTS) {
      return null;
...
async function setContainerConfig(
  ...
  const resp = await callQbase(server, appid, QBASE_SET_CONTAINER_CONFIG, cfg);
  const ret = resp.base_resp?.ret;
  if (ret !== undefined && ret !== 0) {
    throw new QbaseError(
      ret,
...
function buildTransportUnavailablePayload(toolName: string, action?: string) {
  return buildJsonToolResult({
    ...
    message:
      ...
      "当前 CloudBase MCP 进程未注入 qbase 请求通道（pluginOptions.msgPush.requestFn），无法直连" +
      "（安全边界：腾讯云身份不可调用微信登录态 CGI）。\n" +
      "可用方案：\n" +
      "- 在微信开发者工具 MCP 中使用 cloud_msg_push_query / cloud_msg_push_manage（微信 IDE 已注入 qbase 通道）\n" +
      "- 或由宿主（微信 IDE）在 createCloudBaseMcpServer 时传入 pluginOptions.msgPush.requestFn，并在 pluginsEnabled 中启用 msg-push 插件",
    next_step: {
      tool: toolName,
      action,
  ...
  server.registerTool?.(
    "queryMessagePush",
    {
      ...
      description:
        ...
        "需要微信 IDE 登录态通道（宿主注入 pluginOptions.msgPush.requestFn），独立 CloudBase MCP 运行会返回指引错误。",
      inputSchema: {
        appid: z
          .string()
  ...
  server.registerTool?.(
    "manageMessagePush",
    {
      ...
      description:
        ...
        "需要微信 IDE 登录态通道（宿主注入 pluginOptions.msgPush.requestFn）。",
      inputSchema: {
        appid: z
          .string()
          .describe("小程序 AppID（必填，与微信开发者工具一致；用于选择微信登录态会话）"),
