...
function createQbaseBackendMock(options?: {
  ...
  const requestFn = async (params: {
    ...
    calls.push({ url: params.url, body: params.body });
    if (params.url.endsWith("/getappconfig")) {
      return {
        base_resp: { ret: 0 },
        version,
    ...
    if (params.url.endsWith("/route/getcallbacksupportlist")) {
      return {
        base_resp: { ret: 0 },
        data: JSON.stringify({
    ...
    if (params.url.endsWith("/uploadappconfig")) {
    ...
    if (params.url.endsWith("/getcontainercallbackconfig")) {
      return {
        base_resp: { ret: 0 },
    ...
    if (params.url.endsWith("/setcontainercallbackconfig")) {
      containerQbaseOpen = params.body?.qbase_open === true;
      return { base_resp: { ret: 0 } };
    }
    throw new Error(`unexpected qbase url: ${params.url}`);
  };
...
function createMockServer(requestFn: (params: any) => Promise<MsgPushQbaseResponse>) {
  ...
  const server: ExtendedMcpServer = {
    ...
    logger: vi.fn(),
    pluginOptions: { msgPush: { requestFn } },
    registerTool: vi.fn(
...
describe("manageMessagePush — 幂等与 merge", () => {
  ...
  it("subscribe 无 confirm 时返回 CONFIRM_REQUIRED，不发起写请求", async () => {
    ...
    const uploadCalls = backend.calls.filter((c) => c.url.endsWith("/uploadappconfig"));
  ...
  it("subscribe 重复执行幂等：完全相同的订阅不再 POST", async () => {
    ...
    expect(backend.calls.filter((c) => c.url.endsWith("/uploadappconfig"))).toHaveLength(0);
  });
...
describe("manageMessagePush — ensureCloudFunctionMode", () => {
  ...
  it("云托管模式（qbase_open=true）需确认后切换 qbase_open=false", async () => {
    ...
    const setCalls = backend.calls.filter((c) => c.url.endsWith("/setcontainercallbackconfig"));
    expect(setCalls).toHaveLength(1);
    expect(setCalls[0]!.body!.qbase_open).toBe(false);
    // 保留其他云托管字段
    expect(setCalls[0]!.body!.qbase_container_path).toBe("/push");
  });
});
