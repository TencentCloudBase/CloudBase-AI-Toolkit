...
export function registerHostingTools(server: ExtendedMcpServer) {
  ...
  server.registerTool(
    ...
    {
      title: "上传静态文件",
      description: "上传文件到静态网站托管，仅用于 Web 站点部署。云存储对象上传请使用 manageStorage。\n\n⚠️ 必须上传整个构建目录（如 dist/），不要只上传 index.html。\n\n⚠️ 【子目录部署强制二次确认】部署到子目录前，必须先阅读并完成 `web-development/references/deployment-checklist.md` 中的 mandatory checklist（共 5 步 + 输出确认）。未完成 checklist 前禁止调用此工具。cloudPath 不要前导 '/'（如 'vite-test' 而非 '/vite-test'）。上传后必须用 findFiles 验证文件已存在。",
      inputSchema: {
