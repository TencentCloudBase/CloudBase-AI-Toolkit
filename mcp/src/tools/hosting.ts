...
export function registerHostingTools(server: ExtendedMcpServer) {
  ...
  server.registerTool(
    "uploadFiles",
    {
      ...
      inputSchema: {
        ...
        cloudPath: z.string().optional().describe("静态托管云端文件或文件夹路径，使用托管路径语义而不是完整 URL，且路径相对于托管根目录。部署到根目录时请留空；部署到子目录时请传 `vite-test` 这类不带前导 `/` 的相对路径，不要传 `/vite-test`；具体文件可传 `vite-test/index.html`。上传完整构建目录（通常是 `dist/`），不要只上传 index.html。\n\n**警告**：如果站点最终访问路径是 `/vite-test`，构建前必须同步把 Vite `base` 或其他框架的 `publicPath`、`assetPrefix` 配置为 `./` 或与部署路径一致，并重新构建。未设置将导致 JS/CSS 等资源 404。云存储对象路径请改用 manageStorage。"),
        ...
        })).default([]).describe("多文件上传配置，其中每个 cloudPath 都必须是相对于托管根目录的相对路径，不要以 `/` 开头"),
  ...
  server.registerTool?.(
    "deleteFiles",
    {
      ...
      inputSchema: {
        cloudPath: z.string().describe("托管云端文件或文件夹路径，相对于托管根目录的相对路径，不要以 `/` 开头。例如 `vite-test`、`vite-test/index.html`；若要定位根目录，请传空字符串"),
  ...
  server.registerTool?.(
    "findFiles",
    {
      ...
      inputSchema: {
        prefix: z.string().describe("匹配前缀，相对于托管根目录的相对路径，不要以 `/` 开头。例如空字符串表示根目录前缀，`vite-test` 表示子目录前缀"),
