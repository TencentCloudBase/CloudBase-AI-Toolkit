...
export function registerGatewayTools(server: ExtendedMcpServer) {
  ...
  server.registerTool?.(
    "queryGateway",
    {
      title: "查询 CloudBase 网关域资源",
      description:
        "CloudBase 网关域统一只读入口。通过 action 查询网关域名、访问入口和目标暴露情况。",
      inputSchema: {
        action: z
