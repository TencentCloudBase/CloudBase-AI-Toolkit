...
export function registerFunctionTools(server: ExtendedMcpServer) {
  ...
  server.registerTool?.(
    "queryFunctions",
    {
      title: "查询 CloudBase 云函数域资源",
      description:
        "CloudBase 函数域统一只读入口。通过更自解释的 action 查询 CloudBase 云函数列表、函数详情、执行日志、层、触发器和代码下载地址。" +
  ...
  server.registerTool?.(
    "manageFunctions",
    {
      title: "管理 CloudBase 云函数域资源",
      description:
        "CloudBase 函数域统一写入口。支持创建函数、更新代码、更新配置、调用函数、管理定时跑 / 定时任务 / scheduled job 的 timer 触发器和层绑定。" +
