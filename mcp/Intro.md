...
| 工具标识                  | 功能描述                                  | 核心参数                                                                                     |
|---------------------------|-----------------------------------------|---------------------------------------------------------------------------------------------|
| `uploadFiles`             | 上传文件到静态网站托管                    | `localPath`（选填，本地文件或文件夹路径），`cloudPath`（选填，静态托管相对路径，不要带前导 `/`），`files`（选填，多文件上传配置，路径同样不要带前导 `/`），`ignore`（选填，忽略文件模式） |
| `listFiles`               | 获取静态网站托管的文件列表                | 无                                                                                           |
| `deleteFiles`             | 删除静态网站托管的文件或文件夹            | `cloudPath`（必填，静态托管相对路径，不要带前导 `/`），`isDir`（选填，是否为文件夹，默认为 `false`）     |
