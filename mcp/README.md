...
- `EnvInfo.RuntimeMode`：'postgresql' 或 'nosql'，表示**新业务建议默认使用的后端**（PG 已开通时为 postgresql，否则为 nosql）。
- `EnvInfo.RuntimeBackends`：`\{postgresql, nosql, mysql\}` 三个布尔值，描述当前环境**实际并存的后端**。CloudBase PG 环境一般同时保留 NoSQL Database + Storage（即 postgresql=true 且 nosql=true 是常见组合），它们各自独立运行、互不替代。
- `EnvInfo.RuntimeModeHints`：每个后端对应的 API/工具/skill 提示。

AI 在写业务/权限/存储代码前必须先看这三项：PG 模式下**新**业务推荐 `app.rdb()` + RLS（`managePgDatabase action=execute` 跑 `CREATE POLICY`）+ pgstore（bucket 必须先建，与 Supabase 一致）；**已存在**的 NoSQL 集合 / 旧 storage / `managePermissions(resourceType="noSqlDatabase")` 在 PG 环境下仍然有效，不要为了"统一"强行迁移。**真正不适用**的是 MySQL：当 `RuntimeBackends.mysql === false` 时，`manageSqlDatabase` / `querySqlDatabase` / `relational-database-tool` skill 都不该使用——它们是 MySQL 专属，不是 CloudBase PG。
  ...
  parameters={[
    {
      ...
      type: "string",
      required: true,
      description: `操作类型：init=初始化或绑定 PostgreSQL 上下文；execute=执行已确认的写入 SQL 或 DDL；dryRun=只分析 SQL 风险不执行；planMigration=通过 PreviewPGUserMigrations 预览迁移计划；applyMigration=通过 PushPGUserMigrations 应用迁移；listMigrations=查询已应用的 Migration 列表；migrationDetail=查看单条 Migration 详情；rollbackMigration=回滚指定 Migration 可填写的值: "init", "execute", "dryRun", "planMigration", "applyMigration", "listMigrations", "migrationDetail", "rollbackMigration"`,
    },
    {
      name: "sql",
      type: "string",
      description: `action=execute、dryRun 或 planMigration 使用的 SQL 语句`,
    },
    {
      name: "confirm",
      type: "boolean",
    ...
    },
    {
      name: "instanceId",
      type: "string",
      description: `可选的 PostgreSQL 逻辑实例标识。`,
    },
    {
      name: "defaultSchema",
      type: "string",
      description: `默认 schema，会保存到 PG 上下文中，默认 public。`,
    },
    {
      name: "connectionUri",
      type: "string",
      description: `可选的 PostgreSQL 直连 URI。提供该参数时会进入本地 manual 模式。`,
    },
    {
      name: "bootstrapMode",
      type: "string",
      description: `action=init 的初始化模式。cloud 使用 CloudBase Manager SDK executePGSql；podman/local/manual 用于本地开发路径。 可填写的值: "podman", "local", "manual", "cloud"`,
    },
    {
      name: "projectDir",
      type: "string",
      description: `可选的 cloudbase-pgsql 项目目录，用于本地 action=init。`,
    },
    {
      name: "role",
      type: "string",
  ...
  parameters={[
    {
      ...
      type: "string",
      required: true,
      description: `操作类型：buckets/config=查询存储能力摘要；uploadPlan=生成 HTTP API/SDK 上传方案；objectInfo=生成对象元信息查询方案；signUpload/signDownload=显式的一次性签名 URL 请求占位 可填写的值: "buckets", "config", "uploadPlan", "objectInfo", "signUpload", "signDownload"`,
    },
    {
      name: "bucket",
      type: "string",
...
### `manageFunctions`
CloudBase 云函数统一写入口。支持创建函数、更新代码、更新配置、调用函数、管理定时跑 / 定时任务 / scheduled job 的 timer 触发器和层绑定。如果要创建 cron 定时任务，先用 createFunction 创建函数，再用 createFunctionTrigger 创建 timer 触发器（支持7段cron表达式），deleteFunctionTrigger 删除触发器。危险操作需要显式 confirm=true。
  ...
  parameters={[
    {
      ...
      type: "string",
      required: true,
      description: `写操作类型，例如 createFunction、updateFunctionCode、invokeFunction、deleteFunction、createFunctionTrigger（定时任务 / cron / timer）、deleteFunctionTrigger、attachLayer、detachLayer 可填写的值: "createFunction", "updateFunctionCode", "updateFunctionConfig", "invokeFunction", "deleteFunction", "createFunctionTrigger", "deleteFunctionTrigger", "createLayerVersion", "deleteLayerVersion", "attachLayer", "detachLayer", "updateFunctionLayers"`,
    },
    {
      name: "func",
      type: "object",
...
特别说明：
- rules 模板会自动包含当前 mcp 版本号信息（版本号：2.20.2），便于后续维护和版本追踪
  ...
  parameters={[
    {
      ...
      type: "string",
      required: true,
      description: ` 可填写的值: "listApps", "getApp", "listAppVersions", "getAppVersion"`,
    },
    {
      name: "serviceName",
      type: "string",
      description: `CloudApp 服务名。getApp / listAppVersions / getAppVersion 时必填；重新部署后复用同一个 serviceName 查询版本历史。`,
    },
    {
      name: "searchKey",
      type: "string",
    ...
    },
    {
      name: "buildId",
      type: "string",
      description: `构建 ID。getAppVersion 时可与 versionName 二选一；部署返回 BuildId 后可直接用它轮询状态。`,
    }
  ]}
/>
  ...
  parameters={[
    {
      ...
      type: "string",
      required: true,
      description: ` 可填写的值: "deployApp", "deleteApp", "deleteAppVersion"`,
    },
    {
      name: "serviceName",
      type: "string",
      required: true,
      description: `CloudApp 服务名。deployApp 时复用现有 serviceName 会新增一个部署版本并触发重新部署，而不是删除重建。`,
    },
    {
      name: "filePath",
      type: "string",
      description: `要上传并部署的本地项目根目录或 zip 文件绝对路径。deployApp 时必填；通常传源码所在目录而不是 build 产物目录，构建产物目录请用 buildPath 指定。`,
    },
    {
      name: "appPath",
      type: "string",
    ...
    },
    {
      name: "buildPath",
      type: "string",
      description: `构建产物目录，相对于 filePath，例如 dist 或 build。纯静态 HTML 如果入口文件直接在项目根目录，可省略。`,
    },
    {
      name: "framework",
      type: "string",
      description: `前端框架类型。可选值：vue、react、next、nuxt、vite、angular、static；纯 HTML/静态站点请传 static。 可填写的值: "vue", "react", "next", "nuxt", "vite", "angular", "static"`,
    },
    {
      name: "nodeJsVersion",
      type: "string",
      description: `构建时使用的 Node.js 版本；不传时由 CloudApp 使用默认值。`,
    },
    {
      name: "installCmd",
      type: "string",
      description: `依赖安装命令，例如 npm install；静态资源无需安装依赖时可省略。`,
    },
    {
      name: "buildCmd",
      type: "string",
      description: `构建命令，例如 npm run build；纯静态 HTML 无构建步骤时可省略。`,
    },
    {
      name: "deployCmd",
      type: "string",
      description: `自定义部署命令。通常无需填写，只有在默认部署步骤不满足要求时才传。`,
    },
    {
      name: "ignore",
      type: "array of string",
...
- 本工具不涉及 MySQL；MySQL 数据库权限请走 MySQL 自身的 GRANT/REVOKE 语句（通过 `manageSqlDatabase`）。
