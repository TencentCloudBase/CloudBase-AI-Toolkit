  ...
  parameters={[
    {
      ...
      type: "string",
      required: true,
      description: `操作类型：execute=执行已确认的写入 SQL 或 DDL；dryRun=只分析 SQL 风险不执行；planMigration=通过 PreviewPGUserMigrations 预览迁移计划；applyMigration=通过 PushPGUserMigrations 应用迁移；listMigrations=查询已应用的 Migration 列表；migrationDetail=查看单条 Migration 详情；rollbackMigration=回滚指定 Migration 可填写的值: "execute", "dryRun", "planMigration", "applyMigration", "listMigrations", "migrationDetail", "rollbackMigration"`,
    },
    {
      name: "sql",
      type: "string",
...
特别说明：
- rules 模板会自动包含当前 mcp 版本号信息（版本号：2.23.9），便于后续维护和版本追踪
