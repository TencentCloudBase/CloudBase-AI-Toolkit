...
<tr><td><code>writeNoSqlDatabaseStructure</code></td><td>修改 NoSQL 数据库结构。删除集合操作需要 confirm=true 确认。</td></tr>
...
<tr><td><code>executeWriteSQL</code></td><td>Execute a write SQL statement on the SQL database (INSERT, UPDATE, DELETE, etc.). Whenever you create a new table, you **must** include a fixed `_openid` column defined as `_openid VARCHAR(64) DEFAULT '' NOT NULL` that represents the user and is used for access control. Destructive operations (DROP, DELETE, TRUNCATE) require confirm=true.</td></tr>
...
修改 NoSQL 数据库结构。删除集合操作需要 confirm=true 确认。
...
<thead><tr><th>参数名</th><th>类型</th><th>必填</th><th>说明</th></tr></thead>
<tbody>
<tr><td><code>action</code></td><td>string</td><td>是</td><td>createCollection: 创建集合&lt;br/&gt;updateCollection: 更新集合&lt;br/&gt;deleteCollection: 删除集合（需要 confirm=true） 可填写的值: "createCollection", "updateCollection", "deleteCollection"</td></tr>
<tr><td><code>collectionName</code></td><td>string</td><td>是</td><td>集合名称</td></tr>
<tr><td><code>confirm</code></td><td>boolean</td><td></td><td>删除集合时需要设置为 true 以确认操作</td></tr>
...
<tr><td><code>confirm</code></td><td>boolean</td><td></td><td>Confirmation flag for destructive operations (DROP, DELETE, TRUNCATE). Required for safety.</td></tr>
</tbody>
</table>
...
### `searchKnowledgeBase`
云开发知识库智能检索工具，支持向量查询 (vector)、技能文档 (skills) 和 OpenAPI 文档 (openapi) 查询。

      强烈推荐始终优先使用技能文档 (skills) 或 OpenAPI 文档 (openapi) 模式进行检索，仅当固定文档无法覆盖你的问题时，再使用向量查询 (vector) 模式。

      技能文档 (skills) 查询当前支持 22 个 CloudBase 使用指南文档，分别是：
...
<thead><tr><th>参数名</th><th>类型</th><th>必填</th><th>说明</th></tr></thead>
<tbody>
<tr><td><code>mode</code></td><td>string</td><td>是</td><td>可填写的值: "vector", "skills", "openapi"</td></tr>
<tr><td><code>skillName</code></td><td>string</td><td></td><td>mode=skills 时指定。技能文档名称（CloudBase 使用指南）。 可填写的值: "ai-model-nodejs", "ai-model-web", "ai-model-wechat", "auth-http-api", "auth-nodejs", "auth-tool", "auth-web", "auth-wechat", "cloud-functions", "cloud-storage-web", "cloudbase-platform", "cloudrun-development", "data-model-creation", "http-api", "miniprogram-development", "no-sql-web-sdk", "no-sql-wx-mp-sdk", "relational-database-tool", "relational-database-web", "spec-workflow", "ui-design", "web-development"</td></tr>
