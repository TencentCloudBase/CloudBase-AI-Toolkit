...
### `envQuery`
查询云开发环境相关信息，支持查询环境列表、指定环境详情、安全域名和静态网站托管配置。（原工具名：listEnvs/getEnvInfo/getEnvAuthDomains/getWebsiteConfig，为兼容旧AI规则可继续使用这些名称）当 action=list 时，会按 DescribeEnvs 语义做列表/筛选，标准返回字段为 EnvId、Alias、Status、EnvType、Region、PackageId、PackageName、IsDefault，并支持通过 fields 白名单裁剪这些字段；aliasExact=true 时会按别名精确筛选，避免把前缀相近的环境误当作候选；即使传入 envId，action=list 也只返回摘要，不会返回完整资源明细或 expiry。如需查询某个已知 EnvId 对应环境的详细信息，必须使用 action=info 并传入 envId。action=info 会在可用时补充 BillingInfo（如 ExpireTime、PayMode、IsAutoRenew 等计费字段）。
  ...
  parameters={[
    {
      ...
      type: "string",
      required: true,
      description: `查询类型：list=环境列表/摘要筛选（按 DescribeEnvs 语义筛选；即使传 envId 也只返回 EnvId、Alias、Status、EnvType、Region、PackageId、PackageName、IsDefault，不支持 expiry），info=指定环境详细信息（必须传入 envId，详情中可查看更完整资源字段），domains=安全域名列表，hosting=静态网站托管配置 可填写的值: "list", "info", "domains", "hosting"`,
    },
    {
      name: "alias",
      type: "string",
    ...
    },
    {
      name: "envId",
      type: "string",
      description: `按环境 ID 精确筛选。action=list 时可选且仅返回摘要；action=info 时必填并返回该环境详情。如果任务已提供明确 EnvId 且要求查询详情，请直接使用 action=info + envId`,
    },
    {
      name: "limit",
      type: "integer",
...
<ParameterTable
  parameters={[
    ...
    },
    {
      name: "functionName",
      type: "string",
      description: `函数名称。listFunctionLogs、getFunctionDetail、listFunctionLayers、listFunctionTriggers、getFunctionDownloadUrl 时必填`,
    },
    {
      name: "limit",
      type: "number",
...
<ParameterTable
  parameters={[
    ...
    },
    {
      name: "apiName",
      type: "string",
      description: `mode=openapi 时指定。API 名称。 可填写的值: "functions", "storage", "mysqldb", "auth", "cloudrun"`,
    },
    {
      name: "action",
      type: "string",
