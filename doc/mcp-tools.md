...
### `manageApps`
部署 Web 应用到 CloudBase（构建前后端，部署到独立子域名）。action=getUploadUrl 获取预签名上传 URL（cloud mode 下使用）；action=deployApp 上传源码 ZIP 并触发远端构建部署管道：npm install → npm run build → tcb hosting deploy。部署完成后有独立的 `*.webapps.tcloudbase.com` 子域名，也支持绑定自定义域名。新项目推荐传 framework=static 跳过远端构建。与 manageHosting 对比：manageApps 有独立子域名并支持版本管理；manageHosting 适合已有项目继续使用或作为 fallback。两者均可绑定自定义域名。
