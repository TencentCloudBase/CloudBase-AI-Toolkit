...
<tr><td><code>writeNoSqlDatabaseContent</code></td><td>修改 NoSQL 数据库数据记录。可按 MongoDB updateOne/updateMany 的心智模型理解：部分更新必须使用 $set/$inc/$push 等更新操作符；如果直接传 &#123; field: value &#125; 这类普通对象，底层会把它当作替换内容，存在覆盖整条文档的风险。更新嵌套对象中的某个字段时必须使用点号路径（如 &#123; "$set": &#123; "address.city": "shenzhen" &#125; &#125;），若写成 &#123; "$set": &#123; "address": &#123; "city": "shenzhen" &#125; &#125; &#125; 则整个 address 对象会被替换，同级其他字段将丢失。</td></tr>
...
<tr><td><code>queryPgDatabase</code></td><td>Query PostgreSQL local context. Supports context discovery, schema-qualified object listing, lightweight metadata summaries, and read-only SQL execution.</td></tr>
<tr><td><code>managePgDatabase</code></td><td>Manage local PostgreSQL runtime for CloudBase PG MCP. Supports local bootstrap initialization and write SQL execution with explicit confirmation for destructive operations.</td></tr>
<tr><td><code>getPgSchema</code></td><td>Inspect a specific schema-qualified PostgreSQL object. Returns column definitions, key relationships, index summaries, row-level security status, and policy summaries.</td></tr>
...
<tr><td><code>update</code></td><td>object \| string</td><td></td><td>更新内容(对象或字符串,推荐对象)(update 操作必填)。按 MongoDB 更新语义传入 MgoUpdate：部分更新请使用 $set/$inc/$unset/$push 等操作符，例如 &#123; "$set": &#123; "status": "pending" &#125; &#125;；不要直接传 &#123; "status": "pending" &#125;，否则可能替换整条文档。更新嵌套字段时必须用点号路径，如 &#123; "$set": &#123; "address.city": "shenzhen" &#125; &#125;，不要写成 &#123; "$set": &#123; "address": &#123; "city": "shenzhen" &#125; &#125; &#125;（会替换整个 address 对象）。</td></tr>
...
<thead><tr><th>参数名</th><th>类型</th><th>必填</th><th>说明</th></tr></thead>
<tbody>
<tr><td><code>action</code></td><td>string</td><td>是</td><td>context=get current PostgreSQL context; objects=list schema-qualified objects; metadata=get lightweight table metadata; sql=execute read-only SQL 可填写的值: "context", "objects", "metadata", "sql"</td></tr>
<tr><td><code>sql</code></td><td>string</td><td></td><td>Read-only SQL used by action=sql</td></tr>
<tr><td><code>schema</code></td><td>string</td><td></td><td>Optional schema filter used by action=objects or action=metadata</td></tr>
<tr><td><code>limit</code></td><td>integer</td><td></td><td>Optional summary limit for objects, metadata, or SQL rows. Defaults to 20.</td></tr>
</tbody>
</table>
...
<thead><tr><th>参数名</th><th>类型</th><th>必填</th><th>说明</th></tr></thead>
<tbody>
<tr><td><code>action</code></td><td>string</td><td>是</td><td>init=bootstrap or bind local PostgreSQL context; execute=run write SQL or DDL 可填写的值: "init", "execute"</td></tr>
<tr><td><code>sql</code></td><td>string</td><td></td><td>SQL statement used by action=execute</td></tr>
<tr><td><code>confirm</code></td><td>boolean</td><td></td><td>Explicit confirmation required for destructive SQL.</td></tr>
<tr><td><code>envId</code></td><td>string</td><td></td><td>Optional environment identifier for the local PG context</td></tr>
<tr><td><code>instanceId</code></td><td>string</td><td></td><td>Optional logical instance identifier for the local PG context</td></tr>
<tr><td><code>defaultSchema</code></td><td>string</td><td></td><td>Default schema persisted into PG context. Defaults to public.</td></tr>
<tr><td><code>connectionUri</code></td><td>string</td><td></td><td>Optional direct PostgreSQL connection URI. When provided, bootstrapMode becomes manual.</td></tr>
<tr><td><code>bootstrapMode</code></td><td>string</td><td></td><td>Bootstrap mode for action=init. podman is the default local MVP path. 可填写的值: "podman", "local", "manual"</td></tr>
<tr><td><code>projectDir</code></td><td>string</td><td></td><td>Optional cloudbase-pgsql project directory used by action=init.</td></tr>
</tbody>
</table>
...
<thead><tr><th>参数名</th><th>类型</th><th>必填</th><th>说明</th></tr></thead>
<tbody>
<tr><td><code>objectName</code></td><td>string</td><td>是</td><td>Schema-qualified PostgreSQL object name, for example public.users</td></tr>
</tbody>
</table>
