...
### 让 AI 跑定时任务时经常超时失败怎么办？

典型现象是任务跑到一半中断，出现 `Response timed out (no data received)` 或 `Stream timeout` 之类的提示，而工具调用本身（数据库查询、写入等）都是成功的。

这属于 **AI 会话的流式响应空闲超时**：任务链路太长时，模型在两次工具调用之间的思考间隙超过了空闲阈值，与 MCP 工具本身无关。数据聚合 + 逐个实体生成内容 + 逐条落库这类任务最容易触发。

推荐的做法：

1. **把重活放进定时触发的云函数，而不是让 AI 每次现场跑**

   一次性让 AI 生成好脚本并部署，之后由云函数定时器执行，不再依赖模型在线运行：

   ```
   帮我写一个云函数，每天凌晨 2 点统计前一天的数据并写入数据库集合 xxx，
   配置 timer 触发器，然后部署上去
   ```

   参考 [云函数定时触发器](https://docs.cloudbase.net/cloud-function/trigger)（cron 为 7 位：秒 分 时 日 月 周 年）。

2. **拆分任务**：把"全局汇总"和"逐个用户/逐条明细生成"拆成两个独立任务，单次步骤数越少越不容易超时。

3. **减少工具往返**：能用一条聚合 SQL 算完的，不要先查明细再在外面循环处理；查询务必带 `LIMIT` 和分页，避免单次返回超大结果集。

4. **保证幂等和可续跑**：写入时用确定性的 `_id`（如 `daily-{YYYY-MM-DD}`）覆盖更新，并记录处理进度，这样超时重试不会产生重复数据，也能从中断处继续。

5. **精简最终回复要求**：以数据落库成功为准，不要再要求模型输出长篇总结。
