...
在 WorkBuddy 对话中输入 `/cloudbase` 选择 CloudBase Skill，或在对话界面点击左下角 **连接器** 按钮启用 CloudBase 连接器（参阅[配置指南](../ide-setup/workbuddy)）。
...
**修复**：先在 WorkBuddy 中用 `/cloudbase` 或点"连接器"启用 CloudBase。
...
```
1. 环境准备
   ├── 打开 WorkBuddy → 输入 /cloudbase 或点"连接器"
   ├── 设备码登录
   └── 绑定环境
