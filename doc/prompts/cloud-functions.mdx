...
- **MCP 长耗时镜像部署必须闭环**：当使用 `manageFunctions` 的 `deployFunction` 执行 `cloud` 或 `local` 真实部署时，优先传 `wait=false`，避免单次 Tool Call 长时间阻塞。若返回 `taskId`，不得在状态为 `running` 时结束流程、报告成功或让用户自行等待；必须自动调用 `queryFunctions(action="getFunctionDeployStatus", taskId="...")`，按照返回的进度继续轮询，直到 `succeeded` 或 `failed`。仅达到合理轮询上限时，才可报告“仍在执行”，并附带 `taskId`、当前阶段和最近进度。成功时汇报镜像/构建 ID、函数状态和 Gateway URL；失败时汇报失败阶段、错误码、requestId 和诊断建议。若状态为 `expired`，说明任务超过最长保留时间被终结，云端可能仍在部署，必须用 `getFunctionDetail` 确认实际状态，不得直接判定失败。
