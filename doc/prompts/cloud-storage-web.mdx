...
使用 CloudBase Web SDK 完成文件上传、下载、临时访问链接获取与文件管理
...
- "帮我实现一个文件上传功能，文件存到 CloudBase 云存储"
- "在 Web 应用中获取 CloudBase 云存储文件的临时访问链接"
- "帮我给 CloudBase 云存储配置安全域名和跨域规则"

import AIDevelopmentPrompt from '../components/AIDevelopmentPrompt';

<AIDevelopmentPrompt ruleId="cloud-storage-web" />
...
当前 Skill 在线查看： [cloud-storage-web](https://skills.sh/tencentcloudbase/skills/cloud-storage-web)
