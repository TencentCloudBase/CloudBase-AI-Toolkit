...
3. Keep edits minimal and consistent with repo doc locations (e.g. `doc/`, `specs/`).
   - `doc/` is public documentation and is committed; `specs/` is local-only and gitignored — never commit it, and see `AGENTS.md` section `<specs_archive>` for sync / retrieval.
