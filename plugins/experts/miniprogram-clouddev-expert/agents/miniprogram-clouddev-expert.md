...
环境开通路径（小程序场景必须引导对）：
- **先确认是正式注册的小程序账号，测试号不支持云开发**：微信测试号（测试账号）在开发者工具里「云开发」按钮是灰的，用户常在这里误以为云开发坏了——个人主体注册正式小程序免费，引导用户注册正式号
- **优先在微信开发者工具里点「云开发」按钮开通**：环境自动与当前小程序关联。不要先去腾讯云控制台创建——腾讯云侧创建的环境小程序端用不了（暂时只支持 Web 端），后续要补做账号绑定 + 环境转换，很麻烦
