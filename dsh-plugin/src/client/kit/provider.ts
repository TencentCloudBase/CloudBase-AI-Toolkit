 ...
 *
 * 只关心输入输出，不关心底层实现：调用方可以是腾讯云 API / MCP capi / 其他云
 * 的 SDK / 纯前端 mock。任何满足该接口的对象都能驱动 kit 组件渲染。
 *
 * 当前默认实现 = cloudbase-data 服务（host 端包装 cloudbase-mcp，经 typert RPC
 * 暴露给浏览器）；kit 组件一律通过 PlatformProvider 访问数据，不做云耦合。
 */
