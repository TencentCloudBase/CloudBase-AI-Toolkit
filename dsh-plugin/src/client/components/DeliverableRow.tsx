...
import { URL_TOOLS } from "../../shared/constants.js";
...
export function DeliverableRow(props: DeliverableRowProps): React.ReactElement | null {
  ...
  const hosted = [...nodes]
    .reverse()
    .find((node) => URL_TOOLS.some((tool) => (node.toolName ?? node.name ?? "").includes(tool)));
  const deploy = parseDeploy(hosted?.block ?? props.block);
