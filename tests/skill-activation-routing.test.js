...
const POSTGRESQL_SKILL_FILE = path.join(ROOT_DIR, 'config', 'source', 'skills', 'postgresql-development', 'SKILL.md');
...
describe('skill activation routing contract', async () => {
  ...
  test('keeps the source guideline routing table aligned with activation checks', () => {
    ...
    expect(source).toContain('| CloudBase PostgreSQL / PG | `postgresql-development` | auth-tool, auth-web, web-development, cloud-storage-web, http-api | relational-database-tool, no-sql-web-sdk | PG schema, usernamePassword login, backend/RLS permission model |');
  ...
  test('keeps PG and old-auth guardrails visible in source skills', () => {
    ...
    expect(pgSkill).toContain("app.storage.from('covers').upload('a.png', file)");
    expect(pgSkill).toContain('must **not** repeat the bucket prefix');
  });
