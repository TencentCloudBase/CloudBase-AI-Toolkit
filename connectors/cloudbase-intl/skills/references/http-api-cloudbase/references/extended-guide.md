...
```bash
curl -X POST "https://your-env-id.api.intl.tcloudbasegateway.com/v1/functions/YOUR_FUNCTION_NAME" \
...
1. `https://{envId}.api.intl.tcloudbasegateway.com/v1/rdb/rest/{table}`
2. `https://{envId}.api.intl.tcloudbasegateway.com/v1/rdb/rest/{schema}/{table}`
3. `https://{envId}.api.intl.tcloudbasegateway.com/v1/rdb/rest/{instance}/{schema}/{table}`
...
```bash
# Before URL encoding
curl -X GET 'https://your-env.api.intl.tcloudbasegateway.com/v1/rdb/rest/course?select=name,position&name=like.%张三%&title=eq.文章标题' \
  -H "Authorization: Bearer <access_token>"

# After URL encoding
curl -X GET 'https://your-env.api.intl.tcloudbasegateway.com/v1/rdb/rest/course?select=name,position&name=like.%%E5%BC%A0%E4%B8%89%&title=eq.%E6%96%87%E7%AB%A0%E6%A0%87%E9%A2%98' \
  -H "Authorization: Bearer <access_token>"
```
...
```bash
curl -X POST 'https://your-env.api.intl.tcloudbasegateway.com/v1/rdb/rest/course' \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
...
```bash
curl -X PATCH 'https://your-env.api.intl.tcloudbasegateway.com/v1/rdb/rest/course?id=eq.1' \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -H "Prefer: return=representation" \
  -d '{
...
```bash
curl -X DELETE 'https://your-env.api.intl.tcloudbasegateway.com/v1/rdb/rest/course?id=eq.1' \
  -H "Authorization: Bearer <access_token>"
```
...
```
https://{envId}.api.intl.tcloudbasegateway.com/v1/database/instances/{instance}/databases/{database}/
```
...
base="https://${env}.api.intl.tcloudbasegateway.com"
```
