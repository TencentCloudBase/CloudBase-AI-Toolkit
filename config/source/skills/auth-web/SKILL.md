...
- `auth.signInWithOtp({ phone })` and `auth.signUp({ phone })` use the phone number in a `phone` field, not `phoneNumber`
...
```js
const { data, error } = await auth.signInWithOtp({ email: 'user@your-domain.com' })
...
const emailLogin = await auth.signInWithPassword({ email: 'user@your-domain.com', password: 'pass123' })
...
- Use `phone` or `email` in the sign-up payload; do not invent `phoneNumber`
```js
// Email Otp
const emailSignUp = await auth.signUp({ email: 'new@your-domain.com', nickname: 'User' })
...
const updateEmailResult = await auth.updateUser({ email: 'new@your-domain.com' })
const verifyEmailResult = await updateEmailResult.data.verifyOtp({
  email: 'new@your-domain.com',
  token: '123456',
})
