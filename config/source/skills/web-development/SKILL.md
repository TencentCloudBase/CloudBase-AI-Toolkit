...
- Before calling `uploadFiles` for subdirectory deployment, explicitly confirm all of the following: the build config has been updated to that exact absolute path, the project has been rebuilt after the config change, and the built asset references no longer point to the site root `/`
- If any of those checks fail or are still unknown, do not call `uploadFiles` yet; fix the build config and rebuild first
- `cloudPath` is relative to the hosting root and must not start with `/` (use `vite-test`, not `/vite-test`)
- Upload the full build output directory contents (usually `dist/`), not just `index.html`
