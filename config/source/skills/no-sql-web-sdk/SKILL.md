...
- **Setting `authorId` to the wrong value.** When a collection's security rule uses `doc.authorId == auth.uid`, the `authorId` field in the created document must exactly match the logged-in user's `auth.uid`. After calling `auth.signInWithPassword({ username, password })`, the uid is at `data.user.id` (not `data.user.uid`, `data.uid`, or `data.user.user_metadata?.uid`). If `authorId` does not match `auth.uid`, all update and delete operations on that document will be rejected by the security rule, even for the document's creator.
