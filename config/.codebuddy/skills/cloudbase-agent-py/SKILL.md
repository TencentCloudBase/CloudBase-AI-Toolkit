...
6. **Deploy with manageAgent**: Always use the `manageAgent` MCP tool for CloudBase deployment — it natively supports SSE streaming, session persistence, and Python 3.10 runtime. Pre-install dependencies to `./env` with `--platform manylinux2014_x86_64` before deploying.
