...
[点击调试](https://console.cloud.tencent.com/api/explorer?Product=cynosdb&Version=2019-01-07&Action=DescribeAccountAllGrantPrivileges)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/1003/48100) ，本接口取值：DescribeAccountAllGrantPrivileges。 |
...
示例值：cynosdbmysql-asd45qwe |
| Account | 是 | [InputAccount](/document/api/1003/48097#InputAccount) | 账号信息  
示例值：{"AccountName": "andy","Host": "%"} |

## 3\. 输出参数

| 参数名称 | 类型 | 描述 |
| --- | --- | --- |
| PrivilegeStatements | Array of String | 权限语句  
示例值： \["GRANT CREATE, ALTER ON _._ TO 'andy'@'%'","GRANT DROP, EXECUTE ON `andy1`.\* TO 'andy'@'%'","GRANT UPDATE, SHOW VIEW ON `andy2`.`user1` TO 'andy'@'%'"\] |
| GlobalPrivileges | Array of String | 全局权限  
示例值：\["CREATE","ALTER"\] |
| DatabasePrivileges | Array of [DatabasePrivileges](/document/api/1003/48097#DatabasePrivileges) | 数据库权限  
示例值：\[{"Db": "db1","Privileges": \["DROP","EXECUTE"\]}\] |
| TablePrivileges | Array of [TablePrivileges](/document/api/1003/48097#TablePrivileges) | 数据库表权限  
示例值：\[{"Db": "db1","Privileges": \["UPDATE","SHOW VIEW"\],"TableName": "user1"}\] |
