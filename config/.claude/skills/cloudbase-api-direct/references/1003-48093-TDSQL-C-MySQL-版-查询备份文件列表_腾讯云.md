...
[点击调试](https://console.cloud.tencent.com/api/explorer?Product=cynosdb&Version=2019-01-07&Action=DescribeBackupList)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/1003/48100) ，本接口取值：DescribeBackupList。 |
...
示例值：cynosdbpg-ilgo90hu |
| Limit | 否 | Integer | 返回数量，取值范围(0,100\]  
示例值：10 |
| Offset | 否 | Integer | 记录偏移量，取值范围\[0,INF)  
示例值：0 |
| DbType | 否 | String | 数据库类型，取值范围:  
-   MYSQL
  
示例值：MYSQL |
| BackupIds.N | 否 | Array of Integer | 备份ID  
示例值：\[172365\] |
| BackupType | 否 | String | 备份类型，可选值：snapshot，快照备份； logic，逻辑备份  
示例值：snapshot |
| BackupMethod | 否 | String | 备份方式，可选值：auto，自动备份；manual，手动备份  
示例值：auto |
| SnapShotType | 否 | String | 快照类型，可选值：full，全量；increment，增量  
示例值：full |
| StartTime | 否 | String | 备份开始时间  
示例值：2022-01-01 12:00:00 |
| EndTime | 否 | String | 备份结束时间  
示例值：2022-01-01 14:00:00 |
| FileNames.N | 否 | Array of String | 备份文件名，模糊查询  
示例值：\["sls\_20241028221204"\] |
| BackupNames.N | 否 | Array of String | 备份备注名，模糊查询  
示例值：\["一区备份"\] |
| SnapshotIdList.N | 否 | Array of Integer | 快照备份Id列表  
示例值：\[1883726,1883728\] |
| BackupRegion | 否 | String | 备份地域  
示例值：ap-beijing |
...
| 参数名称 | 类型 | 描述 |
| --- | --- | --- |
| TotalCount | Integer | 总共备份文件个数  
示例值：1 |
| BackupList | Array of [BackupFileInfo](/document/api/1003/48097#BackupFileInfo) | 备份文件列表 |
...
| 错误码 | 描述 |
| --- | --- |
| FailedOperation.GetBackupStrategyError | 获取备份策略失败，请稍后重试。如果持续不成功，请联系客服进行处理。 |
