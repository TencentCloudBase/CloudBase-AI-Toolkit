...
[点击调试](https://console.cloud.tencent.com/api/explorer?Product=cynosdb&Version=2019-01-07&Action=CreateBackup)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/1003/48100) ，本接口取值：CreateBackup。 |
...
示例值：cynosdbpg-45knmnra |
| BackupType | 否 | String | 备份类型, 可选值：logic，逻辑备份；snapshot，物理备份  
示例值：logic |
| BackupDatabases.N | 否 | Array of String | 备份的库, 只在 BackupType 为 logic 时有效  
示例值：\["db1"\] |
...
```
https://cynosdb.tencentcloudapi.com/?Action=CreateBackup
...
| OperationDenied.BackupVaultTaskConflictError | 备份保险箱资源 {{1}} 处于{{2}}中无法进行{{3}}的操作 |
| OperationDenied.ClusterTaskConflictError | 集群 {{1}} 正在进行{{2}}操作，无法执行{{3}}操作 |
| OperationDenied.InstanceGrpTaskConflictError | 实例组 {{1}} 正在进行{{2}}操作，无法执行{{3}}操作 |
| OperationDenied.InstanceTaskConflictError | 实例 {{1}} 正在进行{{2}}操作，无法执行{{3}}操作 |
