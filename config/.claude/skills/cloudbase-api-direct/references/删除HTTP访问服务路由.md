...
删除HTTP访问服务域名或者路由，可批量删除多条path路由或者删除所有域名及path路由。
...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | [公共参数](/document/product/705/16277?!preview&preview_docmenu=1&lang=cn&!document=1)，本接口取值：DeleteHTTPServiceRoute。 |
...
| Domain | 是 | String | 删除的域名<br/>示例值：xxx.\*\*\*\*\*\*\*\*\*\*\*\*\*\*\*.cn |
| Paths.N | 否 | Array of String | 删除的路径列表，不传则表示删除域名和所有路由<br/>示例值：["/api/v1"] |
