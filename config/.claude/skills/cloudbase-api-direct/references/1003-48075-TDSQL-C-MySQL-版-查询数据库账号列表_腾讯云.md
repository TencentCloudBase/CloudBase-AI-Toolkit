...
[点击调试](https://console.cloud.tencent.com/api/explorer?Product=cynosdb&Version=2019-01-07&Action=DescribeAccounts)
...
| 参数名称 | 必选 | 类型 | 描述 |
| --- | --- | --- | --- |
| Action | 是 | String | [公共参数](/document/api/1003/48100) ，本接口取值：DescribeAccounts。 |
...
示例值：cynosdbmysql-on5xw0ni |
| AccountNames.N | 否 | Array of String | 需要过滤的账户列表  
示例值：\[admin\] |
...
该参数已废用  
示例值：MYSQL |
| Hosts.N | 否 | Array of String | 需要过滤的账户列表  
示例值：\[0.0.0.0\] |
| Limit | 否 | Integer | 限制量  
示例值：10 |
| Offset | 否 | Integer | 偏移量  
示例值：0 |
| AccountRegular | 否 | String | 模糊匹配关键字(同时匹配AccountName和AccountHost，返回并集结果，支持正则)  
示例值：“andy" |
...
| 参数名称 | 类型 | 描述 |
| --- | --- | --- |
| AccountSet | Array of [Account](/document/api/1003/48097#Account) | 数据库账号列表  
示例值：\[{"AccountName": "tom","Description": "AccountDescription","CreateTime": "2020-09-22 00:00:00","UpdateTime": "2020-09-22 00:00:00","Host": "%","MaxUserConnections": 0}\] |
| TotalCount | Integer | 账号总数量  
示例值：5 |
