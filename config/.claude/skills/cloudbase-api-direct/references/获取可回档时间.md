...
| 参数名称 | 必选 | 类型 | 描述 |
|---------|---------|---------|---------|
| Action | 是 | String | [公共参数](/document/product/705/16277?!preview&preview_docmenu=1&lang=cn&!document=1)，本接口取值：DescribeRestoreTime。 |
...
| InstanceId | 是 | String | 需要回档实例ID |

## 3. 输出参数

| 参数名称 | 类型 | 描述 |
|---------|---------|---------|
| RestoreTimes | Array of String | 可回档时间列表|
| RestoreTimeRanges | Array of [RestoreTableTimeRange](/document/product/705/16289?!preview&preview_docmenu=1&lang=cn&!document=1#RestoreTableTimeRange) | 任意时间回档列表<br/>注意：此字段可能返回 null，表示取不到有效值。|
...
```
https://tcb.tencentcloudapi.com/?Action=DescribeRestoreTime
