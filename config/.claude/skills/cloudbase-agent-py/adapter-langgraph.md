...
agent = LangGraphAgent(
    ...
    enable_agui_patch=True,  # Optional: Enable AG-UI compatibility (default: True)
)
```
...
### 4. Enable AG-UI Patch

```python
# ✅ Correct: Enable for frontend compatibility
agent = LangGraphAgent(
    name="my-agent",
    graph=workflow,
    enable_agui_patch=True  # Default, but be explicit
)

# ⚠️ Caution: Disable only if you know what you're doing
agent = LangGraphAgent(
    name="my-agent",
    graph=workflow,
    enable_agui_patch=False  # May break frontend
)
```
...
**Solution**: Enable AG-UI patch:

```python
agent = LangGraphAgent(
    name="my-agent",
    graph=workflow,
    enable_agui_patch=True  # Critical for frontend
)
```
