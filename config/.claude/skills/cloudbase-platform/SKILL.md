...
2. **Environment Management (via manageEnv)**:
   ...
   **Creating an environment with specific resources:**
   ...
   - Region is selectable: pass `region` (e.g. `region="ap-guangzhou"`) to choose where the environment is created. It is applied as the **`X-TC-Region` request context**, not as a CreateEnv body field — so do **not** put `Region` inside `params`. Omit it to use the current session region (`cloudBaseOptions.region` → `TCB_REGION` → site default). Equivalent CLI: `tcb env create --region ap-guangzhou`.
